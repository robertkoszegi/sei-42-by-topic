...
class NewOrderPage extends React.Component {
  ...
  handleAddToCart = (incoming_item) => {
    let itemAlreadyExistsInCart = this.state.lineItems.some(
      (obj) => obj.item.name === incoming_item.name
    );
    if (itemAlreadyExistsInCart) {
      ...
      this.setState({
        lineItems: this.state.lineItems.map((obj) =>
          obj.item.name === incoming_item.name
            ? { ...obj, qty: obj.qty + 1 }
            : obj
        ),
      });
      ...
      this.setState({
        lineItems: [...this.state.lineItems, { qty: 1, item: incoming_item }],
      });
    }
  };
