...
2. (optional) Ignoring the new "description" field, make the "add skill" button work.


## Submitting the form without refresh
...
### Discussion: Send Form Data to server

Our contact form lab requires us to send this data over to the server, and this afternoon, we will discuss setting up a server and DB for React to talk to.

But as a brief preview, if we wanted addSkill to send our form data to a server, what would we have to do:
  - on the frontend?
  - on the backend?
...
## Optional content Pt. 1: Using a newSkill object.
...
## Optional Content Pt. 2: Validating Form Inputs
